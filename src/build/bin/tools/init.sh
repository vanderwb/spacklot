...
if [[ -n $SPACKLOT_SETUP_DEPLOY ]]; then
    return
fi
...
if [[ ${SPACKLOT_UPSTREAM_MODULES:-true} == true ]]; then
    SPACKLOT_MODULE_FLAGS=--upstream-modules
fi

# Make sure all config variables are exported
for cfg_var in ${!SPACKLOT_*}; do
    export $cfg_var
done
...
if [[ -f $SPACKLOT_ENV_BUILD/.publock ]]; then
    if [[ $(cat $SPACKLOT_ENV_BUILD/.publock) != $SPACKLOT_LOCK_PID ]]; then
        >&2 echo "Error: Someone else is publishing changes; unsafe to proceed."
        exit 1
    fi
fi

if [[ $SPACKLOT_CLEAN != true ]]; then
    tsecho "Sanitizing user environment" ${quiet_mode:+9999}
    export TMPDIR=${SPACKLOT_TMPROOT}/$USER/temp; mkdir -p $TMPDIR
    SPACKLOT_CLEAN=true $SPACKLOT_ROOT_DEPLOYMENT/spack/bin/clean_bash $0 "$@"
    exit $?
elif [[ -z $SPACK_ENV ]]; then
    set -e
    export my_host=$(hostname)
    export my_env_type=${SPACKLOT_ENV_TYPE:-build}
    export start_time=${SPACKLOT_DEPLOYMENT_TIME:-$(date +%y%m%dT%H%M)}

    tsecho "Activating Spack $my_env_type environment" ${quiet_mode:+9999}

    if [[ -f $SPACKLOT_ROOT_ENVS/$my_env_type/spack.yaml ]]; then
        spack env activate $SPACKLOT_ROOT_ENVS/$my_env_type
    else
